...
XYZ（含多帧轨迹）的元素列支持元素符号、1–118 的原子序数及混合写法。支持的笛卡尔输入坐标块也可读取原子序数；生成坐标仍使用元素符号。读取 MOL2 时，会结合原子名称与类型区分常见小写 GAFF 类型和名称相近的元素。
...
文件夹菜单也提供 **Rename**、**Export Folder**；远程文件夹还可使用 **Download to Local**。文件夹下载与导出会保留子目录、空目录、隐藏文件和二进制内容。Download to Local 遇到同名项目时为新副本另取名称，保留已有项目。
...
### 2.5. 在 App 之间拖放文件与文件夹

1. 打开 Local 或连接 SSH 服务器，进入目标目录。
2. 从“文件”或其他 App 将文件、文件夹拖入 Magpie 工作区。侧栏收起时仍可拖入。
3. Magpie 将内容复制到松手时打开的目录；在 SSH 工作区中会上传至服务器。已有项目保留原名，导入副本使用不重复的名称。
4. 要复制到其他 App，从 Magpie 拖出文件或文件夹，放到支持接收的 App 中。远程项目会下载后交付，源项目仍保留。

整文件夹传输保留目录结构，上传和下载均显示进度。取消向外拖动不会启动导出下载。在 Magpie 内将文件拖到文件夹上会移动该文件；放到不接受移动的工作区位置则取消拖动。
...
SSH 已连接时，切换 App 或锁屏会申请临时后台执行时间，以维持共享连接及其读取任务。可用时间由系统决定，不保证无限期保持连接。

实时活动已开启且可用时，锁屏卡片显示喜鹊图标与 **SSH connected**。返回 Magpie、主动断开、切换服务器或后台时间到期时，活动结束。锁屏卡片不会延长系统提供的后台时间。

返回前台后，Magpie 通过 SFTP 检查原连接。连接健康时继续使用原 shell；失效时更新连接状态，保留普通终端历史，并提供手动重连说明。文件传输期间会延后检查。

长时间计算请使用服务器常规作业调度或会话管理工具。实时活动表示连接状态，并不保证远程任务在 SSH 断开后仍会继续运行。
...
1. 选择 **Add atom**。
2. 点击元素符号打开周期表，选择元素。
3. 在 **Bond Order** 中选择 Single、Partial Double、Double 或 Triple。
...
选择 **Add group**，在 **Group / Structure** 中选择 **Functional Groups** 或 **Structures**。官能团可在空白处独立放置、从已有原子拖出连接，或替换氢等末端原子。Structures 中的结构模板只在空白处整块放置，不支持拖拽连接或末端原子替换。
...
在兼容的 iPad 和 Apple Pencil 上，激活 Add atom 或官能团／结构工具：

- **悬停**：落笔前预览位置。元素显示替换落点，官能团显示连接原子，结构模板显示完整模型；预览不改变分子。
- **双击笔身**：在 Add atom 激活时，按 Single → Partial Double → Double → Triple → Single 切换键级。
- **轻捏**：元素模式打开周期表，片段模式打开 Group / Structure 菜单；再次轻捏可关闭。

悬停、双击和轻捏分别需要支持该功能的硬件。优化锁定编辑时不响应 Pencil 编辑动作；没有 Pencil 时仍可使用触摸与鼠标。原子和片段添加的响应优化适用于所有输入方式，包括向大型结构追加片段。
...
两套力场均使用当前结构和键级。同一画布中未相连的多个分子会作为整体优化，并计入分子间相互作用。含过渡金属配位键的结构选择 UFF。MMFF94s 支持已识别的稠合芳香体系，包括更大的合法结构，但仍要求化学上有效的拓扑。UFF 对缺少受支持参数的元素会报错，不会借用其他元素的参数。
...
Mem GB 表示总内存。生成 ORCA 输入时，Magpie 按核心数换算为 `%maxcore`。**Additional keywords** 用于补充关键词，ORCA 对适用方法提供独立的 **AuxJ** 和 **AuxC** 菜单，可同时使用两组选项；**AutoAux** 为全局选项，仅输出一次。生成预览中可检查对应关键词。

隐式溶剂列表提供更多常用溶剂，在支持的程序之间切换时保留所选溶剂。选择溶剂模型与溶剂后检查预览；**None** 表示不选择溶剂。
...
控件下方的文本可直接编辑。手动修改后，文本会保留，不再随后续参数变化自动重生成。点击 **Reset** 仅清除手动文本修改，按当前控件重新生成，并保留全部参数选项与文件名。切换 Format 会开始新的模板。
...
1. 在 Geometry Editor 中打开结构，选择 **Generate → ORCA → Job → GOAT**。
2. 在 **Mode** 中选择标准 GOAT、ENTROPY、EXPLORE、REACT、DIVERSITY 或 COARSE。
3. 按需填写 **Workers**、**Max. global iterations**、**Temperature** 和 **Energy window**。
4. 检查预览并保存输入，在自己的 ORCA 环境中运行采样计算。Magpie 负责输入准备与结果分析，不在设备上运行 ORCA。
5. 保留最终输出及同名配套的 `.finalensemble.xyz`，用于查看结果。

Workers、Temperature 和 Energy window 可留空显示 **Default**，生成时省略对应关键词，沿用 ORCA 默认值。输入中的温度与结果面板中用于探索占比的温度滑条相互独立。COARSE 模式需要在可编辑预览中补充片段定义。
...
密集网格中的普通采样点会缩小并降低不透明度，减少对曲面和等高线的遮挡。当前选中点保持醒目，所有采样数据点仍然保留。
...
将 GOAT 的 `.out` 和配套 `.finalensemble.xyz` 放在同一个 Local 或 Remote 目录，再打开 `.out`。例如，`sample.out` 应与 `sample.finalensemble.xyz` 配套。输出提供最终构象表，ensemble 提供对应几何；初始优化轨迹或搜索过程中的中间集合不能代替最终构象集合。

在 **Conformation** 中使用构象菜单、左右箭头或滑条，也可点击任一图表中的柱子。三维结构与两张图的选中项同步变化。ORCA 构象编号从 **0** 开始，与 Magpie 的原子编号分开。

- **Population** 显示百分比，**Relative energy** 的单位为 kcal/mol。
- iPad 左侧为占比、右侧为能量；iPhone 上方为占比、下方为能量。
- 每次打开文档时，**Temperature** 默认为 298.15 K，范围为 0–500 K。调整时根据相对能量和输出简并度重新计算归一化玻尔兹曼占比，不改变当前构象。
- 0 K 时仅最低能量构象具有占比；多个最低能量构象同能时按简并度分配。
- 输出中的原始百分比保留在 Details 的 **ORCA population** 中。由于输出能量经过舍入，重新计算的占比可能略有差异。拖动温度滑条不会重新运行 GOAT，也不会修改保存的计算结果。
...
可用鼠标或触控板拖选终端文本，再使用右键菜单或 **Command–C** 复制。**Ctrl–C** 用于中断前台命令，不是复制；**Esc** 可传给 vi 等终端程序。新输出到来时仍保留文本选择。

鼠标滚轮与触控板双指滚动可浏览普通 shell 历史。在 vi 等全屏终端程序中，滚动按该程序支持的鼠标或方向键方式传入。

锁屏后的连接行为与重连方式见[切换 App、锁屏与重新连接](#34-切换-app锁屏与重新连接)。
...
单个文件或传输的整棵目录树上限为 512 MiB，文件夹传输不支持符号链接。CUBE 每个文件支持一个标量数据集，最多 2000 万个网格点。可重新生成较粗网格的 CUBE，或导出较短轨迹供设备查看。
...
### 14.10. 为什么锁屏后 SSH 断开了？

后台执行时间由系统临时提供。返回 Magpie 后等待连接检查，若提示连接失效，请手动重连。保留的终端历史不代表旧 shell 仍在运行。详见[第 3.4 节](#34-切换-app锁屏与重新连接)。

### 14.11. 为什么 GOAT 结果打不开？

打开最终 GOAT 输出，并将匹配的 `.finalensemble.xyz` 放在旁边。确认构象数量、元素顺序和能量来自同一次计算，不使用优化轨迹或中间构象集合代替。详见[第 8.6 节](#86-goat-构象集合)。
