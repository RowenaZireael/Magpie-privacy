...
Magpie 是运行于 iPhone 和 iPad 的分子可视化与几何处理程序，可读取分子坐标和计算输出，编辑结构、进行本地力场优化，并生成 ORCA、Gaussian 及坐标文件。通过 SSH 连接，可以访问远程计算目录并调用 Multiwfn 分析波函数。
...
8. [ORCA 分子动力学](#8-orca-分子动力学)
9. [查看计算结果](#9-查看计算结果)
10. [播放轨迹](#10-播放轨迹)
11. [CUBE 表面可视化](#11-cube-表面可视化)
12. [分子轨道与快速 ESP](#12-分子轨道与快速-esp)
13. [终端与交互式 Multiwfn](#13-终端与交互式-multiwfn)
14. [设置](#14-设置)
15. [常见问题](#15-常见问题)
...
导入并打开 Gaussian 或 ORCA 输出文件。在 **Results** 中切换结构轨迹、计算详情、收敛图、振动和分析面板。对于 ORCA 的 IRC 和扫描计算，将配套坐标及轨迹文件与主输出放在同一目录。
...
| 计算输入 | `.inp`、`.gjf`、`.com` | 打开几何结构；使用 Open as Text 编辑输入文本 |
| 计算输出 | Gaussian、ORCA 文本输出，常见扩展名为 `.log`、`.out` | 查看结构、能量、优化、振动、IRC、扫描及电子激发结果 |
...
在服务器上安装 Multiwfn，并将它加入对应 shell 的运行环境。两种二进制波函数格式还需要相应的转换程序：
...
Generate 中的 **Charge** 和 **Mult** 用于量子化学输入；力场中的原子电荷根据当前化学拓扑分配，与这两个输入字段分开处理。

## 7. 生成输入文件与坐标文件

**Generate** 使用编辑器当前的几何结构生成文件。打开时默认选择 ORCA，可在 **Format** 中切换 ORCA、Gaussian、XYZ、MOL2、CIF 或 PDB。
...
Mem GB 表示总内存。生成 ORCA 输入时，Magpie 按核心数换算为 `%maxcore`。**Additional keywords** 用于补充关键词。Gaussian 提供 **Chk** 选项；ORCA 对适用方法提供辅助基组选项。
...
IRC 可选择 Both、Forward 或 Backward，并设置对应程序的路径参数。TDDFT 中通过 **Excited states** 设置请求的激发态数目。
...
4. Gaussian 设置步长和点数；ORCA 设置起点、终点和点数。
...
Gaussian 扫描显示的终值等于初值加上 `(Points − 1) × Step`。二维扫描由两组坐标组成网格，例如第一维 9 个点、第二维 7 个点，对应 63 组坐标组合。
...
打开 Gaussian 或 ORCA 输出文件，显示 **Results**。面板菜单按文件中记录的计算内容提供对应项目。
...
ORCA 的配套文件应与主输出放在同一目录。Magpie 会读取可用的 IRC 路径、合并扫描坐标，以及编号 XYZ 或轨迹 XYZ 来组合结果。Gaussian IRC 与松弛扫描视图使用输出中已接受的计算点。
...
对于 TDDFT 或其他可识别的激发态输出，**UV-Vis Spectrum** 显示以波长为横轴的吸收光谱，波长单位为 nm。图中标出的 Gaussian FWHM 为所用高斯展宽的半峰全宽。
...
### 15.4. 为什么没有振动、光谱或完整扫描点？

打开包含这些结果的主计算输出。坐标文件只提供结构，不包含频率或激发态数据。ORCA IRC 和扫描还应将配套文件放在同一目录，刷新后重新打开输出。

### 15.5. 波函数为什么打不开？

从 Remote 打开，并确认预热环境中能够运行 Multiwfn。`.chk` 检查 `formchk`，`.gbw` 检查 `orca_2mkl`，同时确认源文件所在目录可写。集群分配到期后，重新连接以准备新的环境。
